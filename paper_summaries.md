...
* **Key Idea:** Pre-training on image-caption pairs from internet-scale data enables zero-shot transfer to diverse downstream tasks without task-specific fine-tuning. Contrastive learning between aligned image-text pairs to learn joint representations.
* **Method (Short):** 
  - Dual encoder architecture: image encoder (CNN or ViT) + text encoder (Transformer)
  - Contrastive training: predict ground-truth (image, text) pairs among N×N possible pairs within a batch
  - Natural language prompts for zero-shot classification (e.g., "A photo of a {label}")
* **Key Results:** 
  - Matches ResNet-50 ImageNet accuracy zero-shot without using 1.28M training examples
  - Competitive performance across 30+ vision datasets
...
* **Limitations (IMPORTANT):**
  ...
  - Approach is conceptually straightforward; scaling is the primary innovation, not architectural novelty
